...
def test_get_with_model(run: mocks.Run):
    run.handle(
        ['juju', 'model-constraints', '--format', 'json', '--model', 'test-model'],
        stdout=CONSTRAINTS,
    )

    juju = jubilant.Juju()
    values = juju.model_constraints(model='test-model')
    assert values == {'arch': 'amd64', 'cores': 8, 'mem': 16384}
...
def test_set_with_model(run: mocks.Run):
    ...
    juju = jubilant.Juju()
    values = juju.model_constraints(
        model='test-model', constraints={'arch': 'amd64', 'cores': '8', 'mem': '16384'}
    )
    assert values is None
